...
from dependencies.dependencies import get_events_queue, get_events_status, get_model_registry
...
class EventService:
    ...
    def __init__(
        ...
        self._prompts_manager = PromptManager(Path(__file__).parent.parent.parent.parent/'config.yaml')

    async def add_events(self, events_ids: List[str]):
        ...
        for id in events_ids:
            ...
            riot_event = self.database.get_riot_event_by_id(id)
            if riot_event is None:
                raise NameError(f"Riot event id {id} not found")

            prompt = self.prompts_manager.get_prompt(riot_event.eventName, riot_event.eventData)
            ...
            await self.events_queue.put(processing_riot_event_job)
            await self.events_status.put(processing_riot_event_job)

            self.tracked_events[processing_riot_event_job.id] = processing_riot_event_job

            #TODO add return id
            processing_riot_event_job = self.database.save_processing_riot_event_job(processing_riot_event_job)
