...
tts_router = APIRouter(prefix="/tts", tags=["tts"], dependencies=[Depends(verify_token_flexible)])
...
@tts_router.put("/switch", response_model=SetResponse)
async def switch_tts_model(
    request: SetPayload,
    service: Service,
):
